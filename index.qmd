...
` Answer: High altitude may affect baseball performance because the air is thinner at higher altitudes, which can make it harder to hit the ball. Coors Field is affected by high altitude because it is located at a high altitude in Denver, Colorado. `
...
` Answer: A horizontal bar chart is better than a scatter plot for this data because it is easier to compare the values of the data. `
- When would you choose a vertical bar chart over horizontal?
` Answer: A vertical bar chart is better than a horizontal bar chart when the data is not categorical. `
**Question 2: Aesthetic Mappings**
- What does the color highlighting accomplish in Approach 4?
` Answer: The color highlighting accomplishes in Approach 4 is to highlight the Colorado Rockies stadium. `
- How does position (x/y) compare to color for encoding data?
` Answer: The position (x/y) compares to color for encoding data because the color is used to highlight the Colorado Rockies stadium. `
...
` Answer: The elements that make this visualization suitable for a business presentation are the professional formatting, the clear labels, and the easy to understand and interpret data. `

- Is the annotation on the visualization helpful?  Can you fix its placement?
` Answer: The annotation on the visualization is helpful because it provides a clear and concise explanation of the data. It is also easy to understand and interpret. `
` Answer: Yes, I can fix its placement. `
...
print(f"Ranking change: 1st → 3rd (dropped 2 positions)")
```
